...
device="${3:-0}"
model_type="${4:-Qwen/Qwen3-0.6B}"
other_subsets="${5:-single}"
dataset_name="${6:-P_4}"
survey_size="${7:-16}"

if [ -z "${model_name}" ]; then
  echo "Usage: bash run_ultrafeedback_cautious_context_steering_distill.sh <model_name> [version_name] [device] [model_type] [other_subsets] [dataset_name] [survey_size]"
  echo "Example: bash run_ultrafeedback_cautious_context_steering_distill.sh Qwen3-0.6B cautious_context_steering 0 Qwen/Qwen3-0.6B single P_4 16"
  exit 1
fi

generated_data_dir="${GENERATED_DATA_DIR:-data/${dataset_name}_survey_${survey_size}/${model_type}}"
...
if [[ "${SKIP_PREPARE:-0}" != "1" ]]; then
  python uf_p_4_preprocessing.py "${prepare_args[@]}"
fi
