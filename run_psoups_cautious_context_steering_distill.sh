...
if [[ "${SKIP_PREPARE:-0}" != "1" ]]; then
  "${python_bin}" "${script_dir}/psoups_preprocessing.py" "${prepare_args[@]}"
fi

CUDA_VISIBLE_DEVICES=${device} "${python_bin}" "${script_dir}/eval_psoups_cautious_context_steering_distill.py" \
